...
function HomepageLiveUpdatePanel({
  ...
  const showAction = activeLiveUpdate
    ...
    : true;
  const flyerOnlyMode = Boolean(imageUrl && !showInfo && !showAction);
  ...
  return (
    <div className="flex h-auto min-h-[560px] flex-col overflow-hidden rounded-[30px] bg-white p-5 shadow-xl ring-1 ring-slate-200 sm:p-6 lg:h-[640px] lg:p-5">
      ...
      <div
        className={`mt-4 flex items-center justify-center overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 ${
          flyerOnlyMode
            ? "min-h-0 flex-1"
            : "h-[330px] shrink-0 sm:h-[360px] lg:h-[350px]"
        }`}
      >
        {imageUrl ? (
          <img
            ...
            className={`h-full w-full ${flyerOnlyMode ? "object-contain" : "object-cover"}`}
          />
        ) : (
...
function MiniInfo({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl bg-slate-50 px-4 py-3 ring-1 ring-slate-200 lg:px-3.5 lg:py-2.5">
