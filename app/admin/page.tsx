...
export default function AdminDashboardPage() {
  ...
  const [savingFeaturedLabels, setSavingFeaturedLabels] = useState(false);
  const [featuredLabelsMessage, setFeaturedLabelsMessage] = useState("");
  const [featuredLabelsMessageType, setFeaturedLabelsMessageType] = useState<
    "success" | "error" | ""
  >("");

  const [savingRankingsLabels, setSavingRankingsLabels] = useState(false);
  const [rankingsLabelsMessage, setRankingsLabelsMessage] = useState("");
  const [rankingsLabelsMessageType, setRankingsLabelsMessageType] = useState<
    "success" | "error" | ""
  >("");
  ...
  async function handleSaveHomepageSettings() {
    ...
    async function handleSaveFeaturedSectionLabels() {
    setSavingFeaturedLabels(true);
    setFeaturedLabelsMessage("");
    setFeaturedLabelsMessageType("");

    try {
      const payload = {
        featured_section_eyebrow: homepageForm.featured_section_eyebrow || null,
        featured_section_title: homepageForm.featured_section_title || null,
        featured_section_subtitle: homepageForm.featured_section_subtitle || null,
        updated_at: new Date().toISOString(),
      };
      ...
      setFeaturedLabelsMessage("Featured Tournament section updated successfully.");
      setFeaturedLabelsMessageType("success");
      await loadHomepageSettings();
    } catch (error) {
      setFeaturedLabelsMessage(
        `Failed to save Featured Tournament section. ${formatError(error)}`
      );
      setFeaturedLabelsMessageType("error");
    } finally {
      setSavingFeaturedLabels(false);
    }
  }

  async function handleSaveRankingsSectionLabels() {
    setSavingRankingsLabels(true);
    setRankingsLabelsMessage("");
    setRankingsLabelsMessageType("");

    try {
      const payload = {
        rankings_teams_title: homepageForm.rankings_teams_title || null,
        rankings_players_title: homepageForm.rankings_players_title || null,
        updated_at: new Date().toISOString(),
      };
      ...
      setRankingsLabelsMessage("Rankings section updated successfully.");
      setRankingsLabelsMessageType("success");
      await loadHomepageSettings();
    } catch (error) {
      setRankingsLabelsMessage(
        `Failed to save Rankings section. ${formatError(error)}`
      );
      setRankingsLabelsMessageType("error");
    } finally {
      setSavingRankingsLabels(false);
    }
  }
    ...
    <main className="min-h-screen bg-[#f5f7fb] text-slate-900">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-6 sm:px-6 lg:px-8">
        <div className="rounded-3xl bg-white p-6 shadow-md ring-1 ring-slate-200">
          ...
          <p className="mt-3 max-w-4xl text-sm leading-7 text-slate-600">
            This section controls the homepage Featured Tournament block. First,
            choose which tournament is featured on homepage. Then use Match Center
            to add the next match and latest result. Use Tournament Teams to link
            the participating teams. The heading, title, and subtitle below control
            only the text shown above that homepage section.
          </p>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            ...
            <button
              onClick={handleSaveFeaturedSectionLabels}
              disabled={savingFeaturedLabels}
              className="inline-flex h-12 items-center justify-center rounded-2xl bg-emerald-600 px-5 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {savingFeaturedLabels ? "Saving..." : "Save Featured Tournament Section"}
            </button>

            <p className="text-sm text-slate-500">
              This saves only the Featured Tournament section text labels.
            </p>
          </div>

          {featuredLabelsMessage ? (
            <MessageBox type={featuredLabelsMessageType}>
              {featuredLabelsMessage}
            </MessageBox>
          ) : null}
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-6 sm:px-6 lg:px-8">
        <div className="rounded-3xl bg-white p-6 shadow-md ring-1 ring-slate-200">
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
            <button
              onClick={handleSaveRankingsSectionLabels}
              disabled={savingRankingsLabels}
              className="inline-flex h-12 items-center justify-center rounded-2xl bg-emerald-600 px-5 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {savingRankingsLabels ? "Saving..." : "Save Rankings Section"}
            </button>

            <p className="text-sm text-slate-500">
              This saves only the Rankings section titles.
            </p>
          </div>

          {rankingsLabelsMessage ? (
            <MessageBox type={rankingsLabelsMessageType}>
              {rankingsLabelsMessage}
            </MessageBox>
          ) : null}
          ...
          >
            {playersWatch.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-500">
                No player watch cards added yet. Click <span className="font-semibold text-slate-700">Add New</span> to create the first card.
              </div>
            ) : null}
        ...
        <div className="rounded-3xl bg-white p-6 shadow-md ring-1 ring-slate-200">
          ...
          <div className="mt-6 space-y-4">
            {communityCards.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-500">
                No community cards added yet. Use the buttons above to add Leadership, Advisory Body, or Member cards. Added cards will appear here for editing before you save.
              </div>
            ) : null}
      ...
      <section className="mx-auto max-w-7xl px-4 pb-6 sm:px-6 lg:px-8">
        <div className="rounded-3xl bg-white p-6 shadow-md ring-1 ring-slate-200">
          ...
          <p className="mt-3 max-w-4xl text-sm leading-7 text-slate-600">
            This controls the homepage layout. “Show” decides whether a section appears
            on the public homepage. “Order” decides where it appears from top to bottom.
            Lower numbers appear earlier. Example: Hero = 1, Featured Tournament = 2,
            Rankings = 3.
          </p>
