...
function findBestTeamIdByName(teamName: string | undefined, teams: TeamRow[]) {
  ...
  const tokenMatch = teams.find((item) => {
    ...
    const matchedWords = targetWords.filter((word) => word.length > 2 && name.includes(word));
...
function parseDateTimeToLocalInput(value: string | undefined) {
  if (!value) return "";
...
function extractStumpsSummary(text: string): ParsedStumpsData {
  ...
  const inningsMatches = [...clean.matchAll(/([A-Z][A-Z\s&.-]+?)\s+(\d+-\d+)\s+in\s+([\d.]+)\s+overs/gi)];
  const teamAName = inningsMatches[0]?.[1]?.trim() || "";
  const teamBName = inningsMatches[1]?.[1]?.trim() || "";

  const summaryMatch = clean.match(
    /Match Summary\s+(.+?)\s+1st Innings Scorecard/i
  );
  let keyPlayers = "";
  ...
  if (summaryMatch?.[1]) {
    ...
    const playerSnippets = [...summaryText.matchAll(/[A-Za-z][A-Za-z\s'.-]*\s+\d+\*?\(\d+\)|[A-Za-z][A-Za-z\s'.-]*\s+\d+-\d+/g)]
      .map((m) => m[0].trim());
  ...
  return {
    ...
    matchTitle: matchTitleMatch?.[1]?.trim() || undefined,
...
export default function AdminMatchesPage() {
  ...
  async function handleScorecardPdfUpload(file: File) {
    try {
      ...
      setMessage(
        "Scorecard uploaded and match details were auto-filled from the STUMPS PDF."
      );
  ...
  const teamBOptions = useMemo(() => {
    return [
      ...
      ...teams
        .filter((item) => item.id !== form.team_a_id)
        .map((item) => ({
          value: item.id,
          label: item.name || "Unnamed Team",
        })),
    ];
  }, [teams, form.team_a_id]);
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
        <div className="overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-900 p-6 text-white shadow-xl sm:p-8">
          ...
          <h1 className="max-w-3xl text-3xl font-bold leading-tight sm:text-5xl">
            Create, update, and publish tournament matches quickly.
          </h1>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-200 sm:text-base">
            Upload a STUMPS match report PDF to auto-fill result details, then review
            and save the record.
          </p>
        </div>
      </section>
        ...
        <div className="rounded-3xl bg-white p-6 shadow-md ring-1 ring-slate-200">
          <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                Teams must come from approved team records. The scorecard upload stores
                the full PDF and also attempts to auto-fill the match form.
              </p>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <FieldSelect
              ...
              onChange={(value) =>
                setForm((prev) => ({
                  ...prev,
                  team_a_id: value,
                  team_b_id: prev.team_b_id === value ? "" : prev.team_b_id,
                }))
              }
            ...
            <FieldSelect
              ...
              onChange={(value) => setForm((prev) => ({ ...prev, team_b_id: value }))}
            ...
            <Field
              ...
              onChange={(value) => setForm((prev) => ({ ...prev, key_players: value }))}
