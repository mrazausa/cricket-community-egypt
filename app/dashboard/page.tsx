...
function getInitial(value?: string | null, fallback = "U") {
...
export default function DashboardPage() {
  ...
  async function loadPage() {
    ...
    const [profileRes, teamRes, playerRes] = await Promise.all([
      ...
      supabase
        ...
        .select("id, team_name, team_code, team_logo_url, status, review_note, created_at")
  ...
  const profileAvatarUrl = useMemo(() => {
    const firstPlayerWithPhoto = playerRegistrations.find((row) => row.player_photo_url);
    return firstPlayerWithPhoto?.player_photo_url || "";
  }, [playerRegistrations]);
...
Do you want to continue?`;
  ...
  async function handleDeleteTeam(row: RegistrationTeamRow) {
    const message =
      row.status === "approved"
        ? `This team registration is already approved.

Deleting it will remove the team registration and linked squad entries from your dashboard.

If this team is already being used in other linked modules, those linked records may need separate admin review.

Do you want to continue deleting this approved team registration?`
        : `Are you sure you want to delete this team registration?

This will remove the team registration and linked squad entries from your dashboard.
...
Do you want to continue?`;
    ...
    const { error: squadDeleteError } = await supabase
      ...
      .delete()
      .eq("registration_team_id", row.id);

    if (squadDeleteError) {
      setActionLoadingId("");
      alert(squadDeleteError.message || "Unable to delete linked squad members.");
      return;
    }
    ...
    if (teamDeleteError) {
      alert(teamDeleteError.message || "Unable to delete team registration.");
      return;
    }
          ...
          <div className="rounded-3xl bg-white p-6 shadow-md ring-1 ring-slate-200">
            <div className="mb-5 flex items-center gap-4">
              {profileAvatarUrl ? (
                ...
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-emerald-50 text-xl font-bold text-emerald-700 ring-2 ring-emerald-100">
            ...
            <div className="space-y-2 text-sm text-slate-600">
              <p><span className="font-semibold text-slate-900">Full Name:</span> {profile?.full_name || "Not updated"}</p>
              <p><span className="font-semibold text-slate-900">Mobile:</span> {profile?.mobile_number || "Not updated"}</p>
              <p><span className="font-semibold text-slate-900">Country:</span> {profile?.country || "Not updated"}</p>
              <p><span className="font-semibold text-slate-900">City:</span> {profile?.city || "Not updated"}</p>
              <p><span className="font-semibold text-slate-900">WhatsApp:</span> {profile?.whatsapp_number || "Not updated"}</p>
            </div>
          ...
          <div className="rounded-3xl bg-white p-6 shadow-md ring-1 ring-slate-200">
            ...
            <div className="mt-4 space-y-3">
              {teamRegistrations.length > 0 ? (
                teamRegistrations.slice(0, 10).map((row) => (
                  <div
                    key={row.id}
                    onClick={() => row.team_code && router.push(`/teams/${row.team_code}`)}
                    className="cursor-pointer rounded-2xl border border-slate-200 p-4 transition hover:border-emerald-300 hover:shadow-sm"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span
                            className={`rounded-full px-3 py-1 text-xs font-semibold ${getStatusBadge(row.status)}`}
                          >
            ...
            <div className="mt-4 space-y-3">
              {playerRegistrations.length > 0 ? (
                playerRegistrations.slice(0, 10).map((row) => (
                  <div
                    key={row.id}
                    onClick={() => row.player_code && router.push(`/players/${row.player_code}`)}
                    className="cursor-pointer rounded-2xl border border-slate-200 p-4 transition hover:border-emerald-300 hover:shadow-sm"
                  >
                    <div className="flex items-start gap-3">
